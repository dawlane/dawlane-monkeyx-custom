...
#define TED_VERSION "1.26 (Dawlane modified)"
...
MainWindow::MainWindow(QWidget *parent) : QMainWindow( parent ),_ui( new Ui::MainWindow ){
    ...
    _ui->buildToolBar->addWidget( _configsWidget );
     ...
     // Dawlane CPU Architecture select: Currntly for MinGW and GCC GLFW2/3 (not angle) builds only
    _marchWidget=new QComboBox;
    _marchWidget->addItem( "Default" );
    _marchWidget->addItem( "32 bit" );
    _marchWidget->addItem( "64 bit" );
    _ui->buildToolBar->addWidget( _marchWidget );
...
#ifdef Q_OS_WIN
    // If the Ted build is for windows then disable the cross compiler option in the menu
     _ui->actionBuildCrossCompileMinGW32->setEnabled(false);
     _marchWidget->setEnabled(true);
#else
    // If the TED build is for OS X or Linux then enable the cross compiler option in the menu
    _ui->actionBuildCrossCompileMinGW32->setEnabled(true);
    _marchWidget->setEnabled(true);
#endif
//  Dawlane Modified CPU architecture
#ifdef Q_OS_MAC
    // Disable the CPU architecture selection on OS X. If the cross compiler menu item is checked then it should be enabled.
    // As far as I am aware 32bit coccoa support is no longer included in the OS X SDK
     _marchWidget->setEnabled(false);
#endif
...
void MainWindow::updateActions(){
    ...
    //Dawlane MinGW cross compiler
    /* Only use the CPU architecture combox on OS X when TED's cross compiler switch is checked.
     * May change this if 32bit compiling is still possible via OS X SDK */
#ifdef Q_OS_MAC
    if(_ui->actionBuildCrossCompileMinGW32->isChecked()){ // The cross compiler menu item must be checked
        if(_targetsWidget->currentText()=="Desktop Game (Glfw3)" || _targetsWidget->currentText()=="Desktop Game (Glfw2)"){
            _marchWidget->setEnabled(true);
         }else{
            // As C++ Tool uses #CC_OPT and non C++ targets don't need it. Then disable the CPU architecture combobox
			_marchWidget->setEnabled(false);
         }
    }
    else
...
// Dawlane modified MinGW Cross compile & CPU Architecture
    QString cmd,march,crossCompile,target=_targetsWidget->currentText(),msg="Building: "+filePath+"...";
    if(_ui->actionBuildCrossCompileMinGW32->isChecked() && (target=="Desktop Game (Glfw2)" || target=="Desktop Game (Glfw3)" || target=="C++ Tool")){
        crossCompile="-mingw32cross";
    }
    if(_marchWidget->isEnabled()){
#ifdef Q_OS_MAC
        if(_ui->actionBuildCrossCompileMinGW32->isEnabled() && _marchWidget->isEnabled() && (target=="Desktop Game (Glfw2)" || target=="Desktop Game (Glfw3)" || target=="C++ Tool")){
            switch(_marchWidget->currentIndex() ){
                case 1:
                    march="-msize=\"32\"";
                    break;
                case 2:
...
// Dawlane modified MinGW Cross compile & CPU architecture
    if( editor->fileType()=="monkey" ){
        if( mode=="run" ){
            cmd="\"${MONKEYPATH}/bin/transcc"+HOST+"\" -target=${TARGET} "+march+" "+crossCompile+" -config=${CONFIG} -run \"${FILEPATH}\"";
        }else if( mode=="build" ){
            cmd="\"${MONKEYPATH}/bin/transcc"+HOST+"\" -target=${TARGET} "+march+" "+crossCompile+" -config=${CONFIG} \"${FILEPATH}\"";
        }else if( mode=="update" ){
            cmd="\"${MONKEYPATH}/bin/transcc"+HOST+"\" -target=${TARGET} "+march+" "+crossCompile+" -config=${CONFIG} -update \"${FILEPATH}\"";
            msg="Updating: "+filePath+"...";
        }else if( mode=="check" ){
            cmd="\"${MONKEYPATH}/bin/transcc"+HOST+"\" -target=${TARGET} "+march+" "+crossCompile+" -config=${CONFIG} -check \"${FILEPATH}\"";
...
// Dawlane Modified MinGW cross compiler menu clicked
void MainWindow::onCrossCompileChecked(){
    // Just call update actions for now
    updateActions();
}
