						...
						if(t_1==String(L"-update",7)){
							...
							if(t_1==String(L"-build",6)){
								...
								if(t_1==String(L"-run",4)){
									...
									if(t_1==String(L"-mingw32cross",13)){
										m_opt_mingw32cross=true;
									}else{
...
void c_TransCC::p_LoadConfig(){
						...
						if(t_3==String(L"ANT_PATH",8)){
							...
							if(t_3==String(L"FLEX_PATH",9)){
								...
								if(t_3==String(L"MINGW_PATH",10)){
									if(!((m_MINGW_PATH).Length()!=0) && FileType(t_path)==2 && (HostOS()==String(L"winnt",5) || HostOS()==String(L"macos",5))){
										m_MINGW_PATH=t_path;
									}
								}else{
									if(t_3==String(L"MINGW_TOOLCHAIN",15)){
										if(!((m_MINGW_TOOLCHAIN).Length()!=0) && (HostOS()==String(L"linux",5) || HostOS()==String(L"macos",5))){
											m_MINGW_TOOLCHAIN=t_path;
										}
									}else{
	...
	if(t_4==String(L"winnt",5)){
		...
		if(t_4==String(L"macos",5)){
			...
			if(m_opt_mingw32cross && m_MINGW_PATH!=String() && (FileType(m_MINGW_PATH+String(L"/bin/",5)+m_MINGW_TOOLCHAIN+String(L"-g++",4))==1 || FileType(m_MINGW_PATH+String(L"/",1)+m_MINGW_TOOLCHAIN+String(L"/bin/",5)+m_MINGW_TOOLCHAIN+String(L"-g++",4))==1)){
				t_path3=t_path3+(String(L":",1)+m_MINGW_PATH+String(L"/bin:",5)+m_MINGW_PATH+String(L"/include:",9)+m_MINGW_PATH+String(L"/lib",4));
				t_path3=t_path3+(String(L":",1)+m_MINGW_PATH+String(L"/i686-w64-mingw32/bin:",22)+m_MINGW_PATH+String(L"/i686-w64-mingw32/include:",26)+m_MINGW_PATH+String(L"/i686-w64-mingw32/lib",21));
				t_path3=t_path3+(String(L":",1)+m_MINGW_PATH+String(L"/x86_64-w64-mingw32/bin:",24)+m_MINGW_PATH+String(L"/x86_64-w64-mingw32/include:",28)+m_MINGW_PATH+String(L"/x86_64-w64-mingw32/lib",23));
			}else{
				m_opt_mingw32cross=false;
			}
			...
			if(t_4==String(L"linux",5)){
				...
				if(m_opt_mingw32cross){
					if(m_MINGW_TOOLCHAIN==String() || (FileType(String(L"/usr/bin/",9)+m_MINGW_TOOLCHAIN+String(L"-gcc",4))!=1 || FileType(String(L"/usr/bin/",9)+m_MINGW_TOOLCHAIN+String(L"-g++",4))!=1)){
						m_opt_mingw32cross=false;
					}
				}
...
void c_TransCC::p_Run(Array<String > t_args){
	...
	if(t_args.Length()<2){
		...
		if(HostOS()==String(L"linux",5) || HostOS()==String(L"macos",5)){
			bbPrint(String(L"TRANS Usage: transcc [-msize=32/64] [-mingw32cross] [-update] [-build] [-run] [-clean] [-config=...] [-target=...] [-cfgfile=...] [-modpath=...] <main_monkey_source_file>",170));
		}else{
			bbPrint(String(L"TRANS Usage: transcc [-msize=32/64] [-update] [-build] [-run] [-clean] [-config=...] [-target=...] [-cfgfile=...] [-modpath=...] <main_monkey_source_file>",154));
		}
...
void c_Builder::p_Make(){
	...
	if(HostOS()==String(L"linux",5) || HostOS()==String(L"macos",5)){
		if(m_tcc->m_opt_mingw32cross && (m_tcc->m_target->m_system==String(L"glfw",4) || m_tcc->m_target->m_system==String(L"stdcpp",6))){
			bbPrint(String(L"....Using MinGW32 Cross Compiler....",36));
			bb_config_ENV_HOST=String(L"winnt",5);
		}else{
...
void c_GlfwBuilder::p_MakeGcc(){
	...
	if(!m_tcc->m_opt_mingw32cross){
		...
		bbPrint(String(L"**** Using MinGW cross compiler ****",36));
		t_dst=String(L"gcc_winnt",9);
		t_xcopts=String(L"MINGW=\"",7)+m_tcc->m_MINGW_TOOLCHAIN+String(L"\" CROSSCOMPILE=1",16);
	...
	if(t_1==-1){
		if(!(((t_msize).Length()!=0) || ((m_tcc->m_opt_msize).Length()!=0))){
			bb_transcc_Die(String(L"**** ERROR: Inconsistant 32/64 detected. Check CC and LD option in your source code ****",88));
		}
		if((t_msize).Length()!=0){
			bbPrint(String(L"**** Warning: Inconsistant 32/64 detected. Check CC and LD option in your source code ****",90));
			t_cc_opts=t_cc_opts.Replace(String(L"-m32",4),String());
			t_ld_opts=t_ld_opts.Replace(String(L"-m32",4),String());
			t_cc_opts=t_cc_opts.Replace(String(L"-m64",4),String());
			t_ld_opts=t_ld_opts.Replace(String(L"-m64",4),String());
		}else{
			bbPrint(String(L"**** Warning: Inconsistant 32/64 detected. Check CC and LD option in your source code ****",90));
			bbPrint(String(L"**** transcc msize set to ",26)+m_tcc->m_opt_msize+String(L". Going a head with that option ****",36));
			t_choice=(m_tcc->m_opt_msize).ToInt();
		}
	}else{
		if(t_1==0){
			String t_2=m_tcc->m_opt_msize;
			if(t_2==String(L"32",2)){
				t_choice=32;
			}else{
				if(t_2==String(L"64",2)){
					t_choice=64;
				}else{
					if(!((t_msize).Length()!=0)){
						bbPrint(String(L"**** CPU msize could not be determined. Using GCC/MinGW compiler defaults. ****",79));
					}
				}
			}
		}else{
			if(!((t_msize).Length()!=0) && t_choice>0){
				bbPrint(String(L"**** 32/64 detected in CC/LD parameters. Overriding transcc msize ****",70));
			}
		}
	}
	if((t_msize).Length()!=0){
		String t_3=t_msize;
		if(t_3==String(L"32",2) || t_3==String(L"64",2)){
			bbPrint(String(L"**** GLFW_DESKTOP_MSIZE set to ",31)+t_msize+String(L". Overriding options in CC/LD and transcc ****",46));
			t_choice=(t_msize).ToInt();
		}else{
			bbPrint(String(L"**** CPU msize could not be determined. Using GCC/MinGW compiler defaults. ****",79));
		}
	}
	if(t_choice>0){
		m_casedConfig=m_casedConfig+String(t_choice);
	}
	...
	if(m_tcc->m_opt_build){
		...
		if(t_choice>0){
			...
			if(((m_tcc->m_opt_msize).Length()!=0) || ((t_msize).Length()!=0)){
				...
				t_cc_opts=t_cc_opts.Replace(String(L"-m64",4),String());
				t_ld_opts=t_ld_opts.Replace(String(L"-m64",4),String());
				t_cc_opts=t_cc_opts+(String(L" -m",3)+String(t_choice));
				t_ld_opts=t_ld_opts+(String(L" -m",3)+String(t_choice));
			}
		}
		String t_4=bb_config_ENV_CONFIG;
		if(t_4==String(L"debug",5)){
			t_cc_opts=t_cc_opts+String(L" -O0",4);
		}else{
			if(t_4==String(L"release",7)){
				t_cc_opts=t_cc_opts+String(L" -O3 -DNDEBUG",13);
			}
		}
		...
		if(HostOS()==String(L"linux",5) || HostOS()==String(L"macos",5)){
			if(m_tcc->m_opt_mingw32cross){
				bbPrint(String(L"Executing MinGW cross compiler",30));
			}else{
...
void c_GlfwBuilder::p_MakeMsvc(){
	...
	if((m_tcc->m_opt_msize).Length()!=0){
		String t_5=m_tcc->m_opt_msize;
		if(t_5==String(L"32",2)){
			t_choice=32;
			t_platform=String(L"/p:Platform=Win32",17);
		}else{
			if(t_5==String(L"64",2)){
				t_choice=64;
				t_platform=String(L"/p:Platform=Win64",17);
			}else{
				bbPrint(String(L"CPU architecture could not be determined. Using 32 bit solution.",64));
				t_choice=32;
				t_platform=String(L"/p:Platform=Win32",17);
			}
		}
		bbPrint(String(L"**** Building ",14)+String(t_choice)+String(L" bit GLFW binary ****",21));
	}
	if(t_choice>0){
		m_casedConfig=m_casedConfig+String(t_choice);
	}
...
void c_StdcppBuilder::p_MakeTarget(){
	...
	if(m_tcc->m_opt_build){
		...
		if(t_3==String(L"winnt",5)){
			if(HostOS()==String(L"linux",5) || HostOS()==String(L"macos",5)){
				if(HostOS()==String(L"linux",5)){
					bbPrint(String(L"Building MinGW32 cross Linux Command Line",41));
					t_OPTS=t_OPTS+(String(L" -Wno-free-nonheap-object -I/usr/",33)+m_tcc->m_MINGW_TOOLCHAIN+String(L"/include -I/usr/local/",22)+m_tcc->m_MINGW_TOOLCHAIN+String(L"/include",8));
					t_OPTS=t_OPTS+(String(L" -I/usr/",8)+m_tcc->m_MINGW_TOOLCHAIN+String(L"/sys-root/mingw/include -I/usr/",31)+m_tcc->m_MINGW_TOOLCHAIN+String(L"/sys-root/",10)+m_tcc->m_MINGW_TOOLCHAIN+String(L"/include",8));
					t_LIBS=t_LIBS+(String(L" -L/usr/",8)+m_tcc->m_MINGW_TOOLCHAIN+String(L"/lib -L/usr/local",17)+m_tcc->m_MINGW_PATH+String(L"/lib",4));
					t_LIBS=t_LIBS+(String(L" -L/usr/",8)+m_tcc->m_MINGW_TOOLCHAIN+String(L"/sys-root/mingw/lib -L/usr/",27)+m_tcc->m_MINGW_TOOLCHAIN+String(L"/sys-root/",10)+m_tcc->m_MINGW_TOOLCHAIN+String(L"/lib",4));
					t_LIBS=t_LIBS+String(L" -lwinmm -lws2_32",17);
				}else{
					bbPrint(String(L"Building MinGW32 cross Mac OS X Command Line",44));
					t_OPTS=t_OPTS+(String(L" -Wno-free-nonheap-object -I",28)+m_tcc->m_MINGW_PATH+String(L"/include -I",11)+m_tcc->m_MINGW_PATH+String(L"/i686-w64-mingw32/include -I",28)+m_tcc->m_MINGW_PATH+String(L"/x86_64-w64-mingw32/include",27));
					t_LIBS=t_LIBS+(String(L" -L",3)+m_tcc->m_MINGW_PATH+String(L"/lib -L",7)+m_tcc->m_MINGW_PATH+String(L"/i686-w64-mingw32/lib -L",24)+m_tcc->m_MINGW_PATH+String(L"/x86_64-w64-mingw32/lib",23));
					t_LIBS=t_LIBS+String(L" -lwinmm -lws2_32",17);
				}
			}else{
				bbPrint(String(L"Building MINGW native Win NT Command Line",41));
				t_OPTS=t_OPTS+String(L" -Wno-free-nonheap-object",25);
				t_LIBS=t_LIBS+String(L" -lwinmm -lws2_32",17);
			}
		}else{
			if(t_3==String(L"macos",5)){
				bbPrint(String(L"Building native Mac OS X Command Line",37));
				...
				if(t_3==String(L"linux",5)){
					bbPrint(String(L"Building native Linux GCC Command Line",38));
		...
		if(HostOS()==String(L"linux",5) || HostOS()==String(L"macos",5)){
			if(m_tcc->m_opt_mingw32cross){
				if(HostOS()==String(L"linux",5)){
					bbPrint(String(L"Executing Cross Compiler MinGW32 Linux",38));
					bbPrint(String(L"Command line /usr/bin/",22)+m_tcc->m_MINGW_TOOLCHAIN+String(L"-g++",4)+t_OPTS+String(L" -o ",4)+t_out+String(L" main.cpp",9)+t_LIBS);
					p_Execute(String(L"/usr/bin/",9)+m_tcc->m_MINGW_TOOLCHAIN+String(L"-g++",4)+t_OPTS+String(L" -o ",4)+t_out+String(L" main.cpp",9)+t_LIBS,true);
				}else{
					bbPrint(String(L"Executing Cross Compiler MinGW32 Mac OS X",41));
					bbPrint(String(L"Command line /bin/",18)+m_tcc->m_MINGW_TOOLCHAIN+String(L"-g++",4)+t_OPTS+String(L" -o ",4)+t_out+String(L" main.cpp",9)+t_LIBS);
					p_Execute(m_tcc->m_MINGW_PATH+String(L"/bin/",5)+m_tcc->m_MINGW_TOOLCHAIN+String(L"-g++",4)+t_OPTS+String(L" -o ",4)+t_out+String(L" main.cpp",9)+t_LIBS,true);
				}
				bbPrint(String(L"Appending .exe to output file",29));
				p_Execute(String(L"mv ",3)+t_out+String(L" ",1)+t_out+String(L".exe",4),true);
			}else{
				if(HostOS()==String(L"linux",5)){
					bbPrint(String(L"Executing native Linux GCC compiler",35));
				}else{
					bbPrint(String(L"Executing native Mac OS GCC compiler",36));
				}
				bbPrint(String(L"Command line g++",16)+t_OPTS+String(L" -o ",4)+t_out+String(L" main.cpp",9)+t_LIBS);
				p_Execute(String(L"g++",3)+t_OPTS+String(L" -o ",4)+t_out+String(L" main.cpp",9)+t_LIBS,true);
			}
		}else{
			bbPrint(String(L"Executing native Win NT MinGW32 compiler",40));
			bbPrint(String(L"Command line g++",16)+t_OPTS+String(L" -o ",4)+t_out+String(L" main.cpp",9)+t_LIBS);
			p_Execute(String(L"g++",3)+t_OPTS+String(L" -o ",4)+t_out+String(L" main.cpp",9)+t_LIBS,true);
		}
		if(m_tcc->m_opt_run){
			if(HostOS()==String(L"linux",5) || HostOS()==String(L"macos",5)){
				if(m_tcc->m_opt_mingw32cross){
					bbPrint(String(L"Executing Window NT Cross compiled file. Should execute if WINE is configured for auto start.",93));
					p_Execute(String(L"\"",1)+RealPath(t_out+String(L".exe",4))+String(L"\"",1),true);
				}else{
...
String bb_transcc_RetrieveOpt(String t_search,String t_line,int t_start,String t_delimiter){
	...
	int t_i3=0;
	String t_opt=String();
	while(t_i0<t_line.Length()){
		t_i1=t_line.Find(t_delimiter,t_i0);
		if(t_i1<0){
			t_i1=t_line.Length();
		}
		t_opt=t_line.Slice(t_i0,t_i1);
		t_i2=t_opt.Find(String(L"=",1),0)+1;
		if(t_i2>1){
			t_i3=t_i2;
		}else{
			t_i3=t_i1;
		}
		if(!((t_search.Compare(t_opt.Slice(0,t_i3)))!=0)){
			if(t_i2>1){
				t_result=t_opt.Slice(t_i2,t_i1);
				break;
			}
			t_result=String(t_i0);
			break;
		}
		t_i0=t_i1+1;
	}
